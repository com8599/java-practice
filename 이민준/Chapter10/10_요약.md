...
그렇다면 언제 이런 명시적인 형 변환을 해도 문제가 없을까. 아래와 같을 때 문제가 없다.
child 를 Parent 타입으로 선언하고 다시 Child 타입으로 바꾸면 된다. 
...
이렇게 하는 이유는 하나의 배열에 다양한 타입의 객체들을 넣고 싶을 때 타입 캐스팅으로 같은 타입을 맞추고 배열에 넣을 수 있기 때문이다.
...
이 때 Child 객체는 Parent 클래스의 instance 이기도 하기 때문에 아래와 같이 작성하면 전부 Parent 가 출력된다.
...
아래와 같이 각 객체들의 printName 메소드를 호출하면 Parent 타입의 배열이라도 원래 고유의 타입의 메소드를 호출하게 된다.
```
	public void objectCastArray() {
		Parent[] parentArrays = new Parent[3];
		parentArrays[0] = new Child();
